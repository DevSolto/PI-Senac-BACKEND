import { Controller, Get, Post, Body, Param, Sse, MessageEvent, Patch, Delete } from '@nestjs/common';
import { EventEmitter2 } from '@nestjs/event-emitter';
import { Observable, fromEvent, filter, map } from 'rxjs';
import { DevicesService } from './devices.service';
import { CreateDeviceDto, UpdateDeviceDto, ReadDeviceDto, DeviceCommandDto } from './dto/device.dto';
...
export class DevicesController {
  ...
  @Post(':id/commands')
  sendCommand(@Param('id') deviceId: string, @Body() commandDto: DeviceCommandDto) {
  ...
  @Post()
  async create(@Body() createDeviceDto: CreateDeviceDto): Promise<ReadDeviceDto> {
    const device = await this.devicesService.create(createDeviceDto);
    return plainToInstance(ReadDeviceDto, device, { excludeExtraneousValues: true });
  }

  @Get()
  async findAll(): Promise<ReadDeviceDto[]> {
    const devices = await this.devicesService.findAll();
    return plainToInstance(ReadDeviceDto, devices, { excludeExtraneousValues: true });
  }
